#!/bin/bash -xe
{
  ...
  chroot $mkRoot /bin/bash -xe <<EOF
    set -x; set -e; cd /install-scripts; export LMBUILD="raspios"; ls; chmod +x *.sh; ./install.sh 0 2 4 6 a; exit
EOF
  ...
  cp -v ./work/$thisArch/$imageName ./release/$thisArch/bbn-os-alpha_${LYSMARINE_VER}-${thisArch}-${cpuArch}.img
